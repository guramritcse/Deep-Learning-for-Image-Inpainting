    ...
    try:  
        ...
        for iteration in range(start_iteration, config['niter'] + 1):
            ...
            if iteration % (config['viz_iter']) == 0:
                ...
                val_losses, val_inpainted_result, val_offset_flow = trainer(val_x, val_bboxes, val_mask, val_ground_truth, False)

                if val_x.size(0) > viz_max_out:
                    viz_images = torch.stack([val_x[:viz_max_out], val_inpainted_result[:viz_max_out],
                                              val_offset_flow[:viz_max_out]], dim=1)
                else:
                    viz_images = torch.stack([val_x, val_inpainted_result, val_offset_flow], dim=1)
